...
- [x] ドロップ後の処理（差し替え / 複数ファイル / 重複回避）
- [ ] ドラッグの受け取り自体（Finder からの実ドラッグ。自動化できず手動確認が必要）
...
### 詰まった点
7. **`addSubview(_:)` をオーバーライドしてはいけない。** ハイライトを常に最前面へ置こうとして
...
8. **Finder からの実ドラッグは自動化できなかった。** CGEvent でのドラッグは Finder のウインドウが
   別 Space にいると AX から見えず、3 回試して断念。ドロップ後のロジックは一時的な env フックで
   実測して確認し、ドラッグの受け取り自体は手動確認に回した。
