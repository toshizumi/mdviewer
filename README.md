...
## サクッと見れる。超軽量Markdownビューアー

macOS 用。`.md` を右クリックして開くだけ。必要なら A4 の PDF にできます。
**編集機能はありません。**
...
**開いているウインドウにファイルを落とすと、そのウインドウの表示が切り替わります。**
別ウインドウが増えないので、次々に読んでいくときに使えます。

- 複数まとめて落とすと、1つ目は落としたウインドウ、残りは新しいウインドウで開きます
- すでに別のウインドウで開いているファイルは、そのウインドウが前に出るだけです（同じファイルが2枚並びません）
- テキストとして読めないファイル（画像など）は受け付けません
